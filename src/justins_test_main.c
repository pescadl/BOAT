...
void initLdma(void)
{
  ...
  static LDMA_Descriptor_t loopDescriptor =
    LDMA_DESCRIPTOR_LINKREL_M2P_BYTE(&sineTable[0],   // Memory source address
                                     ...
                                     SINE_TABLE_SIZE, // Number of bytes per transfer
...
/**************************************************************************//**
 * @brief Setup push button BTN0 as PRS source for DMAREQ0.
 *****************************************************************************/
static void gpioPrsSetup(void)
{
  ...
  /* Configure push button BTN0 as input */
  const int button1 = 8;
  GPIO_PinModeSet(gpioPortD, button1, gpioModeInputPullFilter, 1);

  /* Configure interrupt on push button PB1 for rising edge but not enabled - PRS sensing instead */
  GPIO_IntConfig(gpioPortD, button1, true, false, false);

  /* Enable PRS clock */
  CMU_ClockEnable(cmuClock_PRS, true);

  /* Select GPIO as PRS source and push button BTN0 as signal for PRS channel 1 */
  PRS_SourceSignalSet(1, PRS_CH_CTRL_SOURCESEL_GPIOH, button1, prsEdgePos);
...
void initLdmaPrs(void)
{
  ...
  // Trigger on TIMER0 overflow
  LDMA_TransferCfg_t transferConfig =
    LDMA_TRANSFER_CFG_PERIPHERAL(ldmaPeripheralSignal_PRS_REQ0);
...
int main(void)
{
  ...
  // Timer triggered DAC sinewave
//  initVdac();
//  initLdma();
//  initTimer();

  // Button triggered DAC sinewave
//  initVdac();
//  initLdmaPrs();
//  gpioPrsSetup();
